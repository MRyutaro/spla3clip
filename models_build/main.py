...
def build_death_model():
    ...
    """
    # 正解データのパス
    death_image_dir = os.path.join(TRAIN_DATA_DIR, "death")
    # 不正解データのパス
    other_image_dir = os.path.join(TRAIN_DATA_DIR, "other")
    ...
    # killディレクトリから画像を繰り返し取得
    for image_name in os.listdir(death_image_dir):
        image_path = os.path.join(death_image_dir, image_name)  # 画像のパスを作成
...
def build_start_model():
    ...
    """
    # 正解データのパス
    start_image_dir = os.path.join(TRAIN_DATA_DIR, "start")
    # 不正解データのパス
    other_image_dir = os.path.join(TRAIN_DATA_DIR, "other")
    ...
    # killディレクトリから画像を繰り返し取得
    for image_name in os.listdir(start_image_dir):
        image_path = os.path.join(start_image_dir, image_name)  # 画像のパスを作成
...
def build_finish_model():
    ...
    """
    # 正解データのパス
    finish_image_dir = os.path.join(TRAIN_DATA_DIR, "finish")
    # 不正解データのパス
    other_image_dir = os.path.join(TRAIN_DATA_DIR, "other")
    ...
    # killディレクトリから画像を繰り返し取得
    for image_name in os.listdir(finish_image_dir):
        image_path = os.path.join(finish_image_dir, image_name)  # 画像のパスを作成
    ...
    # processed_imagesをテストデータと訓練データに分割、割合は8:2
    X = [image[0] for image in processed_images]
    Y = [image[1] for image in processed_images]
    X_train, X_test, Y_train, Y_test = train_test_split(X, Y, test_size=0.2, random_state=42)
    ...
    # モデルの保存
    picke_name = "finish_model.pickle"
    with open(os.path.join(MODELS_DIR, picke_name), "wb") as f:
        pickle.dump(clf, f)
